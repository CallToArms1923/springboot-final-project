<template>
	<div class="main-content" :style='{"color":"#3B3E40","padding":"30px 30px 30px","fontSize":"14px"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"border":"0px solid #bababa","padding":"0 10px","margin":"0 0 20px","overflow":"hidden","borderRadius":"0px","flexWrap":"wrap","background":"none","display":"flex","fontSize":"inherit","justifyContent":"space-between"}' :inline="true" :model="searchForm">
				<el-row :style='{"border":"0px solid #e3e3e3","padding":"10px 0","borderRadius":"5px","textAlign":"center","flexWrap":"wrap","background":"#fff","flexDirection":"row","display":"flex","width":"auto","fontSize":"inherit","justifyContent":"flex-start","order":"2"}' >
					...
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","flexWrap":"wrap","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","color":"inherit","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">学校</label>
						<el-input v-model="searchForm.xuexiao" placeholder="学校" @keydown.enter.native="search()" clearable></el-input>
					</div>
				...
				<el-row class="actions" :style='{"border":"0px solid #e3e3e3","padding":"10px 0","margin":"0","color":"#fff","textAlign":"left","flexDirection":"row","display":"flex","justifyContent":"flex-start","borderRadius":"5px","flexWrap":"wrap","background":"#fff","width":"auto","fontSize":"inherit"}'>
					<el-button class="add" v-if="isAuth('toudixinxi','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia13" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"inline-block"}'></span>
						添加
					</el-button>
					<el-button class="del" v-if="isAuth('toudixinxi','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
					...
					<el-button class="btn18" v-if="isAuth('toudixinxi','审核')" :disabled="dataListSelections.length?false:true" type="success" @click="shBatchDialog()">
						<span class="icon iconfont icon-shenhe2" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"inline-block"}'></span>
						审核
					</el-button>

					<el-button class="btn18" v-if="isAuth('toudixinxi','每日投递人数统计')" type="success" @click="chartDialog1()">
			...
			<div :style='{"border":"0px solid #e9e9e9","width":"100%","padding":"0px","margin":"10px 0 0","borderRadius":"5px","background":"rgba(255,255,255,1)"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('toudixinxi','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
							...
							<el-button class="btn8" v-if="isAuth('toudixinxi','面试')" @click="mianshitongzhiCrossAddOrUpdateHandler(scope.row,'cross','否','','','')" type="success">
							...
							<el-button class="edit" v-if=" isAuth('toudixinxi','修改')  && scope.row.sfsh=='待审核' " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('toudixinxi','删除') " type="danger" @click="deleteHandler(scope.row.id )">
		...
		<mianshitongzhi-cross-add-or-update v-if="mianshitongzhiCrossAddOrUpdateFlag" :parent="this" ref="mianshitongzhiCrossaddOrUpdate"></mianshitongzhi-cross-add-or-update>
		...
		<el-dialog
		  ...
		  width="800">
			<div id="toudiriqiChart1" style="width:100%;height:600px;"></div>
		  <span slot="footer" class="dialog-footer">
...
import mianshitongzhiCrossAddOrUpdate from "../mianshitongzhi/add-or-update";
	export default {
		data() {
			return {
				searchForm: {
				...
				mianshitongzhiCrossAddOrUpdateFlag: false,
		...
		components: {
			AddOrUpdate,
			mianshitongzhiCrossAddOrUpdate,
		},
		methods: {
			contentStyleChange() {
			...
			mianshitongzhiCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				...
				this.mianshitongzhiCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','toudixinxi');
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.mianshitongzhiCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.mianshitongzhiCrossaddOrUpdate.init(row.id,type);
				});
			},
...
//统计接口
    chartDialog1() {
      ...
      this.$nextTick(()=>{
        var toudiriqiChart1 = echarts.init(document.getElementById("toudiriqiChart1"),'fresh-cut');
        this.$http({
            ...
            if (data && data.code === 0) {
                ...
                // 使用刚指定的配置项和数据显示图表。
                toudiriqiChart1.setOption(option);
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    toudiriqiChart1.resize();
                };
            }
        });
      })
    ...
    getDataList() {
      ...
      let params = {
        ...
        order: 'desc',
           ...
           if(this.searchForm.xuexiao!='' && this.searchForm.xuexiao!=undefined){
            params['xuexiao'] = '%' + this.searchForm.xuexiao + '%'
          }
    ...
    shBatchHandler(){
      this.$confirm(`是否${this.batchIds.length>1?'一键审核':'审核'}选中数据?`, "提示", {
        ...
        this.$http({
          url: "toudixinxi/shBatch?sfsh="+this.shBatchForm.sfsh+"&shhf="+this.shBatchForm.shhf,
          method: "post",
          data: this.batchIds
        }).then(({ data }) => {
          if (data && data.code === 0) {
	  ...
	  this.$http.get('xueshengjianli/queryByXuehao', {
		...
		if (res.data.code === 0 && res.data.data) {
		  ...
		  this.$http.get('xuesheng/page', {
			params: { xuehao: row.xuehao, page: 1, limit: 1 }
		  }).then(studentRes => {
			if (studentRes.data.code === 0 && studentRes.data.data.list.length > 0) {
			  ...
			  this.onlineResumeData = {
				...res.data.data,
				xingming: student.xingming || row.xingming,
				nianling: student.nianling || '-',
				xingbie: student.xingbie || '-',
				xueli: student.xueli || row.xueli,
				xuexiao: student.xuexiao || row.xuexiao
			  };
	...
	toudixinxistatusChange(e,row){
		if(row.status==0){
			row.passwordwrongnum = 0
		}
		this.$http({
			url:'toudixinxi/update',
			method:'post',
			data:row
		}).then(res=>{
			if(row.status==1){
...
<style lang="scss" scoped>
	...
	.center-form-pv .actions .add {
				...
				text-shadow: 1px 1px 0 rgba(0, 0, 0, 0.2);
				background: linear-gradient(to bottom, #C9B9DB 0%,#B9AFCB 100%),#B9AFCB;
				width: auto;
				min-width: 70px;
				height: auto;
			}
	...
	// 统一表头排序箭头位置 - 强制同行显示
	.main-content .el-table th /deep/ .cell {
		white-space: nowrap !important;
		overflow: hidden;
	}
	.main-content .el-table th /deep/ .caret-wrapper {
	...
	.main-content .el-table th /deep/ .sort-caret {
		width: 0;
		height: 0;
	...
	.main-content .el-table th /deep/ .sort-caret.ascending {
		border-bottom-color: #C0C4CC;
		top: 2px;
	}
	.main-content .el-table th /deep/ .sort-caret.descending {
