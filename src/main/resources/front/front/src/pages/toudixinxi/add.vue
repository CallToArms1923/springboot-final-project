...
<div :style='{"minHeight":"500px","padding":"20px 20px 40px","margin":"0px auto","borderColor":"#d3d3d3","background":"#fff","borderWidth":"0 1px","width":"980px","position":"relative","borderStyle":"solid"}'>
    ...
    >
          ...
          <el-form-item :style='{"width":"100%","padding":"0px","margin":"0 0 20px","background":"none","display":"inline-block"}' label="简历类型" prop="jianlileixing">
            <el-radio-group v-model="ruleForm.jianlileixing" @change="onJianliTypeChange">
              <el-radio :label="0">上传附件简历</el-radio>
              <el-radio :label="1">使用在线简历</el-radio>
            </el-radio-group>
          </el-form-item>
          <el-form-item :style='{"width":"100%","padding":"0px","margin":"0 0 20px","background":"none","display":"inline-block"}' v-if="ruleForm.jianlileixing === 0" label="简历附件" prop="jianlifujian">
            <file-upload
            tip="点击上传简历附件"
            action="file/upload"
            :limit="1"
...
<script>
  export default {
    ...
    // 多级联动参数
      ...
      // 提交
      onSubmit() {
			// 校验：如果选择使用在线简历，先检查是否已填写
			if (this.ruleForm.jianlileixing === 1) {
				return this.$http.get('xueshengjianli/queryByXuehao', {
					params: { xuehao: this.ruleForm.xuehao }
				}).then(res => {
					if (!res.data.data) {
						this.$message.error('请先前往我的简历页面填写在线简历');
						return;
					}
					// 已填写，继续提父流程
					this.submitForm();
		...
		submitForm() {
			...
			// 移除投递时扣减人数的逻辑,改为录取时扣减
			var obj = JSON.parse(localStorage.getItem('crossObj'));
			var table = localStorage.getItem('crossTable');
			// obj.zhaopinrenshu = obj.zhaopinrenshu - this.ruleForm.zhaopinrenshu  // 已移除
			// if(obj.zhaopinrenshu<0){  // 已移除
					...
					if(crossrefid && crossuserid) {
						...
						this.$http.get('toudixinxi/list', {
							...
							if(res.data.data.total>=crossoptnum) {
								...
								// obj.zhaopinrenshu = parseFloat(obj.zhaopinrenshu) - parseFloat(this.ruleForm.zhaopinrenshu)  // 已移除:录取时才扣减
							
								// this.$http.post(table+`/update`,obj).then(res => {});  // 已移除
		...
		// 简历类型切换
		onJianliTypeChange(val) {
			if (val === 1) {
				// 选择在线简历，清空附件
				this.ruleForm.jianlifujian = ''
			}
		},
